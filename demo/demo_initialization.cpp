  ...
  public:
    ...
    FAILED_REASON TryToInitialize(cv::Mat im) {
      ...
      if (!isIm1Set) {
        ...
        // 构造Frame
        frame1Ptr = std::make_shared<Frame>(im1Gray, 0.0, orbExtractor, nullptr, settings.mK, settings.mDistCoef);
        // 打印特征点数量
        LOG(INFO) << "frame1.mvKeysUn.size(): " << frame1Ptr->mvKeysUn.size();
        mvbPrevMatched.clear();
        mvbPrevMatched.reserve(frame1Ptr->mvKeys.size());
        for (size_t i1 = 0; i1 < frame1Ptr->mvKeysUn.size(); i1++) {
          mvbPrevMatched.push_back(frame1Ptr->mvKeysUn[i1].pt);
        }
        isIm1Set = true;
      ...
      frame2Ptr = std::make_shared<Frame>(im2Gray, 1.0, orbExtractor, nullptr, settings.mK, settings.mDistCoef);

      Frame& frame1 = *frame1Ptr;
      Frame& frame2 = *frame2Ptr;
      ...
      if (!isTriangulated) {
        LOG(ERROR) << "Triangulation turn false, triangulation failed. Keep Frame1";
